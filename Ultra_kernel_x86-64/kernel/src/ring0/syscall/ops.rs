...
/// M5d E: la escena 3D con luz, con la ficha en `arg1`. Ver `dev/gpu_trabajo.rs`.
...
/// **El nombre de una orden de `TASK_OP_IOMMU`**, sin el `IOMMU_OP_`: lo dice
/// la pantalla de fallo (`plat::iommu::EN_CURSO`). Sale de las constantes de
/// arriba, asi que no se separa de ellas; una que falte aqui dice `?`.
pub(crate) fn nombre_iommu(op: u64) -> &'static str {
    match op {
        IOMMU_OP_ENCENDER => "ENCENDER",
        IOMMU_OP_APAGAR => "APAGAR",
        IOMMU_OP_CEGAR_GPU => "CEGAR_GPU",
        IOMMU_OP_VER_GPU => "VER_GPU",
        IOMMU_OP_E2_ENCENDER => "E2_ENCENDER",
        IOMMU_OP_E2_APAGAR => "E2_APAGAR",
        IOMMU_OP_TRADUCIR_GPU => "TRADUCIR_GPU",
        IOMMU_OP_PRESTAR_PRUEBA => "PRESTAR_PRUEBA",
        IOMMU_OP_GPU_FUEGO => "GPU_FUEGO",
        IOMMU_OP_GPU_FRONTERA => "GPU_FRONTERA",
        IOMMU_OP_FWSEC_PREPARAR => "FWSEC_PREPARAR",
        IOMMU_OP_FWSEC_TROZO => "FWSEC_TROZO",
        IOMMU_OP_FWSEC_CORRER => "FWSEC_CORRER",
        IOMMU_OP_GSP_PREPARAR => "GSP_PREPARAR",
        IOMMU_OP_GSP_TROZO => "GSP_TROZO",
        IOMMU_OP_GSP_PRESTAR => "GSP_PRESTAR",
        IOMMU_OP_GSP_COMPROBAR => "GSP_COMPROBAR",
        IOMMU_OP_GSP_LIBOS => "GSP_LIBOS",
        IOMMU_OP_GSP_DESPERTAR => "GSP_DESPERTAR",
        IOMMU_OP_GSP_BOOTER => "GSP_BOOTER",
        IOMMU_OP_GSP_ACABAR => "GSP_ACABAR",
        IOMMU_OP_GSP_LEIDO => "GSP_LEIDO",
        IOMMU_OP_GSP_SISTEMA => "GSP_SISTEMA",
        IOMMU_OP_GSP_SECUENCIAR => "GSP_SECUENCIAR",
        IOMMU_OP_GSP_BAR1 => "GSP_BAR1",
        IOMMU_OP_GSP_ESTATICA => "GSP_ESTATICA",
        IOMMU_OP_GSP_OBJETO => "GSP_OBJETO",
        IOMMU_OP_GSP_CONTROL => "GSP_CONTROL",
        IOMMU_OP_GPU_VRAM => "GPU_VRAM",
        IOMMU_OP_GPU_DIRECTORIO => "GPU_DIRECTORIO",
        IOMMU_OP_GPU_RAIZ => "GPU_RAIZ",
        IOMMU_OP_GPU_TRAMO => "GPU_TRAMO",
        IOMMU_OP_GPU_CANAL => "GPU_CANAL",
        IOMMU_OP_GPU_CANAL_ORDEN => "GPU_CANAL_ORDEN",
        IOMMU_OP_GPU_COPIADOR => "GPU_COPIADOR",
        IOMMU_OP_GPU_COPIA => "GPU_COPIA",
        IOMMU_OP_GPU_LEER => "GPU_LEER",
        IOMMU_OP_GSP_GR => "GSP_GR",
        IOMMU_OP_GPU_CANAL_GR => "GPU_CANAL_GR",
        IOMMU_OP_GPU_GR_MEMORIA => "GPU_GR_MEMORIA",
        IOMMU_OP_GPU_GR_MEDIDA => "GPU_GR_MEDIDA",
        IOMMU_OP_GSP_GR_PROMOVER => "GSP_GR_PROMOVER",
        IOMMU_OP_GSP_GR_TRESDE => "GSP_GR_TRESDE",
        IOMMU_OP_GSP_COMPUTO => "GSP_COMPUTO",
        IOMMU_OP_GPU_TRABAJO_GR => "GPU_TRABAJO_GR",
        IOMMU_OP_GPU_SOMBREO => "GPU_SOMBREO",
        IOMMU_OP_GPU_LIENZO => "GPU_LIENZO",
        IOMMU_OP_GPU_LIENZO_LEER => "GPU_LIENZO_LEER",
        IOMMU_OP_GPU_LIENZO_ESCRIBIR => "GPU_LIENZO_ESCRIBIR",
        IOMMU_OP_GPU_BLUR => "GPU_BLUR",
        IOMMU_OP_GPU_FRACTAL => "GPU_FRACTAL",
        IOMMU_OP_GPU_TRIANGULO => "GPU_TRIANGULO",
        IOMMU_OP_GPU_LIMPIAR_3D => "GPU_LIMPIAR_3D",
        IOMMU_OP_GPU_ESCENA => "GPU_ESCENA",
        IOMMU_OP_GPU_RASTER => "GPU_RASTER",
        IOMMU_OP_GPU_COLOR_3D => "GPU_COLOR_3D",
        IOMMU_OP_GPU_DIAG_3D => "GPU_DIAG_3D",
        IOMMU_OP_GPU_GIRO => "GPU_GIRO",
        IOMMU_OP_GSP_DESPEDIR => "GSP_DESPEDIR",
        IOMMU_OP_GSP_CERRAR => "GSP_CERRAR",
