...
pub(crate) fn gpu(dsk: &mut Desktop, p: &bmo::Pantalla, arg: &[u8]) -> After {
    ...
    if arg == b"secuenciador" {
        return super::gspsecuencia::orden(dsk, p);
    }
    if arg == b"init" {
        return super::gspinit::orden(dsk, p);
    }
    if arg == b"bar1" {
        return super::gspinit::orden_bar1(dsk, p);
    }
    if arg == b"estatica" {
        return super::gsprpc::orden(dsk, p);
    }
    if arg == b"objetos" {
        return super::gspobjeto::orden(dsk, p);
    }
    if arg == b"salud" {
    ...
    if arg == b"directorio" {
        return super::gspvram::orden_directorio(dsk, p);
    }
    if arg == b"tramo" {
    ...
    if arg == b"oro" {
        return super::gspgr::orden_oro(dsk, p);
    }
    if arg == b"computo" {
        return super::gspcomputo::orden(dsk, p);
    }
    if arg == b"sombreo" {
        return super::gspcomputo::orden_sombreo(dsk, p);
    }
    if arg == b"lienzo" {
        return super::gspcomputo::orden_lienzo(dsk, p);
    }
    if arg == b"blur" {
        return super::gspcomputo::orden_blur(dsk, p);
    }
    if arg == b"fractal" || arg == b"panel" {
        return super::gspcomputo::orden_fractal(dsk, p);
    }
    if arg == b"triangulo" {
        return super::gspcomputo::orden_triangulo(dsk, p);
    }
    if arg == b"3d" {
        return super::gspcomputo::orden_3d(dsk, p);
    }
    if arg == b"giro" {
        return super::gspcomputo::orden_giro(dsk, p);
    }
    if let Some(resto) = arg.strip_prefix(b"aguante") {
        if resto.is_empty() || resto[0] == b' ' {
            return super::gspaguante::orden(dsk, p, resto);
        }
    }
    if let Some(resto) = arg.strip_prefix(b"imagen") {
        if resto.is_empty() || resto[0] == b' ' {
            return super::gspcomputo::orden_imagen(dsk, p, resto);
        }
    }
    ...
    if arg == b"color" {
        return super::gspcomputo::orden_color3d(dsk, p);
    }
    if arg == b"raster" {
        return super::gspcomputo::orden_raster(dsk, p);
    }
    if arg == b"escena" {
        return super::gspcomputo::orden_escena(dsk, p);
    }
    let op = match arg {
        b"" => None,
...
pub(crate) fn report_gpu(s: &mut Output, rayo: Option<bmo::CuentasRayo>) {
    ...
    super::gspsecuencia::fila(s);
    super::gspinit::fila(s);
    super::gsprpc::fila(s);
    super::gspobjeto::fila(s);
    super::gspsalud::fila(s);
    super::gspvram::fila(s);
