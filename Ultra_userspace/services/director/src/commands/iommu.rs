...
pub(crate) fn motivo(m: u32) -> &'static [u8] {
    match m {
        ...
        bmo::IOMMU_NO_YA_DESPIERTO => b"el booter ya corrio en este arranque (el motivo de verdad es el de antes): otra vez pide un arranque nuevo",
        bmo::IOMMU_NO_GPU_CALIENTE => b"la WPR2 ya estaba EXTENDIDA antes de nuestro booter: otro booter corrio antes y el nuestro no cargaria",
        ...
        bmo::IOMMU_NO_COPIA => b"sin copiador, la copia ya se hizo en este arranque, o la ficha no es la de nuestro canal",
        ...
        super::gspcanalgr::NO_CANALGR_NEGADO => b"el GSP-RM contesto pero NO dio el canal de GR0: su NV_STATUS, en la fila `canal gr`",
        bmo::IOMMU_NO_GR_MEMORIA => b"los buferes de GR no caben en lo que G2 mapea, el tramo no esta mapeado, o ya se mapearon en este arranque",
        ...
        bmo::IOMMU_NO_BLUR => b"los trabajos de GR necesitan el lienzo (`lienzo`) antes y una ficha del canal de GR0; o hay uno en marcha",
        ...
        bmo::IOMMU_NO_VOLCADO => b"el volcado por la 3060 no se puede: sin la copia de L1d3, sin `gpu init` (BAR1 fisica), o el lienzo no es del escritorio",
        super::gspvolcado::NO_VOLCADO_MAL => b"la 3060 copio el escritorio pero no todas las muestras salieron: mira la fila `volcado`",
        ...
        bmo::IOMMU_NO_BODRIO => b"TOMA TU BODRIO: el juez del SASS, en la puerta del kernel, no dejo subir un programa del BSF a la 3060 (la regla y la instruccion, en la cabina)",
        ...
        super::gspcomputo::NO_BLUR_SUBIR => b"un trozo de la pantalla no se pudo subir al lienzo",
        bmo::IOMMU_NO_LIENZO => b"el lienzo necesita el primer sombreador (`sombreo`) antes y una ficha del canal de GR0; o ya se pinto en este arranque",
        bmo::IOMMU_NO_LIENZO_PREPARAR => b"el lienzo no se presto por la IOMMU, sus PTE no estaban vacias, o el tramo no se releyo: no se toco el timbre",
        super::gspcomputo::NO_LIENZO_MAL => b"el programa del lienzo se lanzo pero no pinto los 16384 pixeles: la fila `lienzo`",
        bmo::IOMMU_NO_SOMBREO => b"el sombreador necesita S3 (`trabajogr`) antes, una ficha del canal de GR0; o ya se corrio en este arranque",
        bmo::IOMMU_NO_SOMBREO_PREPARAR => b"el programa, el QMD o las ordenes no se releyeron igual por PRAMIN: no se toco el timbre",
        super::gspcomputo::NO_SOMBREO_MAL => b"el sombreador se lanzo pero no escribio sus 32 palabras: la fila `sombreo`",
        bmo::IOMMU_NO_COMPUTO => b"S1 necesita el contexto de oro (`oro`) antes, o el computo ya se pidio en este arranque",
        bmo::IOMMU_NO_TRABAJO_GR => b"S3 necesita el computo, una ficha del canal de GR0 (chid 2); o ya se hizo en este arranque",
        bmo::IOMMU_NO_TRABAJO_GR_PREPARAR => b"las ordenes del GR no se releyeron igual por PRAMIN: no se toco el timbre",
        super::gspcomputo::NO_COMPUTO_NEGADO => b"el RM no creo AMPERE_COMPUTE_B: su NV_STATUS, en la fila `computo`",
        super::gspcomputo::NO_TRABAJO_SIN_FICHA => b"sin la ficha del canal de GR0 ni su lista en la tabla: no se toca el timbre (`fichagr`)",
        super::gspcomputo::NO_TRABAJO_MAL => b"el timbre sono pero el motor grafico no pago el semaforo: la fila `gr trabajo`",
        bmo::IOMMU_NO_GR_TRESDE => b"G4 necesita G3 (PROMOTE_CTX) antes, o el objeto 3D ya se pidio en este arranque",
        super::gspgr::NO_GR_NO_PROMOVIDO => b"el RM no acepto los buferes (PROMOTE_CTX): su NV_STATUS, en la fila `gr oro`",
        super::gspgr::NO_GR_TRESDE_NEGADO => b"el RM no creo AMPERE_B: su NV_STATUS, en la fila `gr oro` -- el contexto de oro no salio",
        super::gspgr::NO_GR_SIN_BUFERES => b"sin los buferes de G0 (`gpu gr`), o no caben en los 32 MiB de G2",
        super::gspgr::NO_GR_MAPEO_MAL => b"las tablas de G2 se escribieron pero alguna entrada no se releyo igual",
