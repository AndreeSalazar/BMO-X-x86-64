...
| El SONIDO: mono, 5.1, 7.1 y 3D | ABIERTA (9) | `plan/PLAN_EL_SONIDO.md` (22-09, peticion del propietario: *"todo tipo de sonido, como tablas de guias... ultra dinamico... con amplificador como DaVinci"*). El APARATO ya esta (audifono reclamado y tubo abierto en el Ryzen el 22-09); esto es la ONDA: tablas de formatos, frecuencias exactas contra fraccionarias y repartos de canales con su `wChannelConfig`, y la cadena fuente -> remuestreo -> panorama -> mezcla -> **ganancia con medidor y limitador** -> tubo. La ley: ni una frecuencia ni un canal escrito en el codigo, los declara el aparato. Dato del metal: el alt elegido del audifono 7.1 es ESTEREO (192 B/ms), y nadie ha mirado si ofrece otros. **22-09: S4c, el MAESTRO** --la ultima etapa en el kernel y su panel en el escritorio, sin reclamar el aparato-- en codigo; falta el metal |
| LA MESA: el control de sonido como app | ABIERTA (7) | `plan/PLAN_LA_MESA.md` (22-09, peticion del propietario: *"una app ventana, el control de sonido MAESTRO, inspirado en Premiere, y que el audio se divida por completo"*). El modelo de PISTAS con nombre (mudo, solo, medidor propio, derivar a fichero) esta HECHO en `bmo-amplificador::mesa` (M0, 13 pruebas); falta el productor, la ventana y que cada programa pida su pista. La correccion honesta: Windows SI tiene volumen por app desde Vista; lo que no da de serie es **grabar cada app en su propia pista**, y eso es lo que aqui se persigue |
...
| **VISION** (24-09): EL VIGILANTE -- una tarjeta PCIe propia que hace de frontera del DMA | **VISION**, sin fecha de codigo | Del propietario, el dia que la IOMMU contesto: un procesador RISC-V con SU propia RAM, en el que los aparatos escriben; el procesa, valida y ALIMENTA la RAM real (mapea y desmapea en caliente), y la RAM del PC nunca la toca un aparato ajeno. Lo que aporta sobre la IOMMU: la IOMMU juzga DONDE escribe un aparato; el VIGILANTE puede juzgar QUE escribe (descriptores bien formados, tramas validas, hashes). Y la ironia que el propietario le puso, que es la regla: **el vigilante tiene su VENTANA siempre expuesta**, y BMO-X lo lee TODO por ella, como CABINA lee el kernel -- lo contrario del GSP, que es el vigilante de Nvidia y no se deja leer. Existe por partes (DPU/SmartNIC como BlueField, los bounce buffers de SEV/TDX, Qubes). Sus tres limites: solo vigila lo que cuelga DETRAS de el (switch PCIe o peer-to-peer), paga el ancho de banda dos veces (sobra para USB, red y disco, no para una grafica), y el mismo hace DMA -- asi que la IOMMU sigue haciendo falta, con UN solo aparato que vigilar y cuyo firmware es propio. Encaja con la DPU de la fila de abajo |
