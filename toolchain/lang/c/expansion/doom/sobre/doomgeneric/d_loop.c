...
void TryRunTics (void)
{
    ...
    /* == VIGIA (2026-09-09): LA UNICA FASE QUE PUEDE ESPERAR =============
     *
     * El 09-09 DOOM dejo de imprimir a los ~25 s, **sin fallo y sin mensaje**,
     * y `main` es un `for(;;)`: no puede salir. O sea que se queda dentro de un
     * tick, y de las cuatro fases de `doomgeneric_Tick` esta es la unica que
     * espera a algo.
     *
     * ** El bucle de abajo TIENE salida --si `I_GetTime()` avanza un tic, sale--
     * asi que solo se queda si **el reloj se para**. Este contador lo dice con
     * nombre y numeros en vez de dejar la pantalla congelada y callada.
     *
     * [!] Cuesta un `int` y una comparacion por vuelta, y no imprime NUNCA
     * mientras el reloj ande. Un vigia que habla cuando todo va bien es un
     * vigia que se apaga. */
    {
    int vueltas_vigia = 0;
	...
	NetUpdate ();
        ...
        lowtic = GetLowTic();
        vueltas_vigia = vueltas_vigia + 1;
        if (vueltas_vigia == 20000) {
            printf("[vigia] TryRunTics ATASCADO: lowtic=%d gametic=%d counts=%d"
                   " tiempo=%d entertic=%d enjuego=%d
",
                   lowtic, gametic, counts, I_GetTime(), entertic,
                   (int)PlayersInGame());
        }
