...
{ 
    ...
    /* ** EL CEPO, AQUI Y NO EN EL PRIMER Tick (2026-08-30).
     *
     * Estaba armado alrededor del primer `doomgeneric_Tick()`, y el metal del
     * 30-08 dijo que la rotura YA EXISTE al salir de aqui:
     *
     *     [heap] tras P_SetupLevel: ASIGNADOR +1889056 t8  1/1590 rotos
     *
     * O sea que el cepo vigilaba una ventana en la que el destrozo ya estaba
     * hecho. Un instrumento apuntado al sitio equivocado no dice menos: dice
     * nada, y encima parece que funciona.
     *
     * [!] Camina los ~1590 bloques en cada paso por la zona, y `P_SetupLevel`
     * hace muchos. Esto TARDA, y es a proposito: es una corrida de
     * diagnostico, no una partida. Se desarma solo al primer roto. */
    { void bmo_vigilar(int encendido); bmo_vigilar(1); }
    P_SetupLevel (gameepisode, gamemap, 0, gameskill);
    { void bmo_vigilar(int encendido); bmo_vigilar(0); }    
    displayplayer = consoleplayer;		// view the guy you are playing    
    gameaction = ga_nothing;
    // == SONDA BMO-X, TEMPORAL =========================================
    //
    // ** `Z_CheckHeap()` MATA, y es el UNICO sitio de toda la partida que
    // mira el monton. Por eso DOOM se muere siempre aqui: no porque este paso
    // rompa nada, sino porque es el primero que abre los ojos.
    //
    // El metal del 14-08 lo dejo claro: con el destrozo del arranque ya
    // remendado antes del primer fotograma, DOOM jugo 120 fotogramas y 171
    // tics y **se murio igual aqui**. O sea que hay un SEGUNDO destrozo entre
    // medias, y con una funcion que mata no se puede ni verlo ni pasar de el.
    //
    // El auditor mira lo mismo con el mismo criterio, lo dice con bloque y
    // numero, remienda lo remendable y **deja seguir**. Se vuelve a poner
    // `Z_CheckHeap` el dia que el monton salga limpio.
    { void bmo_auditar(char *donde); bmo_auditar("tras P_SetupLevel"); }
